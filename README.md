...
- [Usage](#usage)
- [Download a tagged release](#download-a-tagged-release)
...
- Dashboard with reusable UI templates for easy customisation.
...
The goal is to keep the storage layer simple with SQLite while still being flexible and open.
...
Release builds are published when a tag such as `v1.2.0` is pushed. Download the assets from the repository's GitHub Releases page, or use GitHub CLI:
...
Run each application separately. The native CLI and dashboard are independent programs and do not need to be started together.

### Downloaded Linux release

Extract and run the native CLI:

```bash
tar -xzf giftyfy-linux.tar.gz
./giftyfy
```
...
| --- | --- |
| ![Dashboard](assets/dashboard.png) | ![Items view](assets/items-view.png) |

| Tables view | Console app |
| --- | --- |
| ![Tables view](assets/table-view.png) | ![Console app](assets/console-app.png) |
...
├── assets/                # contains assets used iin github and dashboard. Screenshots for the github, other assets refrenced by dashboard
...
├── sqlite3.o              # Compiled object file for SQLite.
├── build/                 # build artifacts (not in repo, generated by python)
└── dist/                  # where your dashboard launch file lives
```
